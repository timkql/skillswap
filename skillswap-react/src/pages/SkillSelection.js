  ...
  const handleContinue = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/profile/${user.uid}/skills`, {
        learning_skills: selectedSkills.map(skill => skill.name)
      });

      if (response.data) {
        navigate('/teaching-skills');
      } else {
        throw new Error('Failed to save learning skills');
      }
    } catch (error) {
      ...
      <Container maxWidth="md">
        <Box sx={{ mt: 4, mb: 4 }}>
          ...
          <Typography variant="subtitle1" color="text.secondary" align="center" sx={{ mb: 4 }}>
            Select up to 5 skills (you can change these later)
          </Typography>
